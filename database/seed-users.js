#!/usr/bin/env node
// Добавляет / обновляет пользователей в базе данных.
// Безопасно запускать повторно — использует ON CONFLICT DO UPDATE.
// Запуск: node database/seed-users.js
...
async function run() {
  try {
    for (const u of USERS) {
      ...
      console.log(`✓ ${u.role.padEnd(10)} ${u.email}  (пароль: ${u.password})`)
    }
