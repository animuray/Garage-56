...
import { Check, X, Search, LogOut, ChevronDown } from 'lucide-react'
...
// ─── Complete modal (same as AppointmentsPage) ─────────────────────────────────
function CompleteModal({ apt, onClose, onSave }: {
  apt: Appointment
  onClose: () => void
  ...
  const getValidationError = (): string | null => {
    ...
    if (!liters.trim() || isNaN(litersNum) || litersNum <= 0) return 'Укажите количество литров'
    if (litersNum > 999) return `Количество литров (${litersNum}) слишком большое`
    ...
    try {
      await onSave({
        ...
        serviceRecord: {
          ...
          notes, total, servicePrices: parsed,
        },
      })
